	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw ai lisho n vai faze falta nenhuma @${num.split('@')[0]} feiaõ👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					premium: '*𝗗𝗘𝗦𝗖𝗨𝗟𝗣𝗘 𝗘𝗦𝗧𝗘 𝗥𝗘𝗖𝗨𝗥𝗦𝗢 𝗣𝗔𝗥𝗔 𝗨𝗦𝗨A𝗥𝗜𝗢 𝗣𝗥??𝗠𝗜𝗨𝗠 𝗘𝗦𝗣??𝗖I𝗙I𝗖𝗢!!*',
		    ...
		    case 'map':
			    ...
			    reply(mess.wait)
					...
					if (args[0] === 'buka') {
						...
						reply(`\`\`\`✓Sucesso no Fechamento do Grupo. Tome anúncio\`\`\` *${groupMetadata.subject}*`)
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, você não é um administrador.Ta com poder nenhum agr  :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
